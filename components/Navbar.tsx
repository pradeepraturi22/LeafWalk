...
async function loadCustomerSession() {
    try {
      if (typeof window !== 'undefined') {
        const cached = window.sessionStorage.getItem(NAVBAR_SESSION_CACHE_KEY)
        if (cached) {
          const parsed = JSON.parse(cached) as { ts?: number; user?: NavUser | null; role?: string | null }
          if (parsed?.ts && Date.now() - parsed.ts < NAVBAR_SESSION_CACHE_TTL_MS && parsed.user) {
            setUser(parsed.user)
            setUserRole(parsed.role || parsed.user.role || 'user')
            return true
          }
        }
      }
      ...
      if (result?.user && !result?.requires_profile_completion) {
        ...
        if (typeof window !== 'undefined') {
          window.sessionStorage.setItem(NAVBAR_SESSION_CACHE_KEY, JSON.stringify({
            ts: Date.now(),
            user: nextUser,
      ...
      if (typeof window !== 'undefined') {
        window.sessionStorage.removeItem(NAVBAR_SESSION_CACHE_KEY)
      }
      return false
    } catch {
      return false
