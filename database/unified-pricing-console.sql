...
-- Date-wise pricing for Direct Booking, OTA, and Walk-in (B2C).
...
-- One date-wise base price row per room category + source + day.
...
-- One flat seasonal tour-operator row per season + meal plan.
create unique index if not exists room_rates_unique_tariff_slot
  ...
  where season_id is not null and meal_plan is not null;
