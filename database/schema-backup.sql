-- Leafwalk Resort - Complete Database Schema with OTA & Tour Operator Support
...
-- =============================================
-- ROOMS TABLE (Updated with Categories)
-- =============================================
...
CREATE TABLE IF NOT EXISTS bookings (
  ...
  ota_booking_id UUID REFERENCES ota_bookings(id) ON DELETE SET NULL,
...
CREATE TRIGGER update_ota_integrations_updated_at BEFORE UPDATE ON ota_integrations
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE POLICY "Admins can manage tour operators" ON tour_operators
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM users
            WHERE users.id = auth.uid()
...
-- OTA policies
CREATE POLICY "Admins can manage OTA integrations" ON ota_integrations
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM users
            WHERE users.id = auth.uid()
...
CREATE POLICY "Admins can view OTA bookings" ON ota_bookings
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM users
            WHERE users.id = auth.uid()
...
-- Insert sample rooms with categories
...
ARRAY['/rooms/deluxe-room.jpg', '/rooms/deluxe-2.jpg'])
...
'/rooms/cottage.jpg', 
ARRAY['/rooms/cottage.jpg', '/rooms/cottage-2.jpg'])
ON CONFLICT (slug) DO UPDATE SET 
  category = EXCLUDED.category,
  price = EXCLUDED.price,
  amenities = EXCLUDED.amenities;
...
'/rooms/suite.jpg', 
ARRAY['/rooms/suite.jpg', '/rooms/suite-2.jpg'])
ON CONFLICT (slug) DO UPDATE SET 
  category = EXCLUDED.category,
  price = EXCLUDED.price,
  amenities = EXCLUDED.amenities;
...
COMMENT ON TABLE rooms IS 'Available room types and inventory';
COMMENT ON TABLE bookings IS 'Guest reservations and bookings';
COMMENT ON TABLE tour_operators IS 'Travel agents and tour operators';
COMMENT ON TABLE ota_integrations IS 'Online Travel Agency integrations';
COMMENT ON TABLE ota_bookings IS 'Bookings received from OTA platforms';
