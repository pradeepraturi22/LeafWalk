...
interface DailyPriceOverride {
  date: string; room_category: string
  base_price: number; adjustment: number; final_price: number
}
interface PricingMatrixNight {
  date: string
  base_price: number
...
const PUBLIC_MEAL_INFO: Record<PublicMealPlan, { label: string; desc: string; icon: string }> = {
  EP: { ...MEAL_INFO.EP, label: 'Only Room', desc: 'Room stay without meals' },
  CP: { ...MEAL_INFO.CP, label: 'Including Breakfast', desc: 'Breakfast included daily' },
}
MEAL_INFO.EP = PUBLIC_MEAL_INFO.EP
MEAL_INFO.CP = PUBLIC_MEAL_INFO.CP
...
function validateEmail(e: string) {
  if (!e.trim()) return 'Email address is required'
  if (!EMAIL_RE.test(e.trim())) return 'Enter a valid email address'
}
...
function toDS(d: Date) { return toLocalDateString(d) }
...
function AvailCalendar({ checkIn, checkOut, allRooms }: { checkIn: string; checkOut: string; allRooms: Room[] }) {
  ...
  async function load() {
    setLoading(true)
    try {
      const res = await fetch('/api/check-availability/calendar', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ checkIn, checkOut }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Could not load availability calendar')

      const deluxeDays = data?.categories?.deluxe || []
      const premiumDays = data?.categories?.premium || []
      const dateMap = new Map<string, DayAvail>()

      for (const day of deluxeDays) {
        dateMap.set(day.date, {
          date: day.date,
          deluxeAvail: Number(day.availableRooms || 0),
          deluxeTotal: Number(day.totalRooms || 0),
          premiumAvail: 0,
          premiumTotal: 0,
        })
      }
      ...
      for (const day of premiumDays) {
        ...
        if (existing) {
          existing.premiumAvail = Number(day.availableRooms || 0)
          existing.premiumTotal = Number(day.totalRooms || 0)
        } else {
          dateMap.set(day.date, {
            date: day.date,
            deluxeAvail: 0,
            deluxeTotal: 0,
            premiumAvail: Number(day.availableRooms || 0),
            premiumTotal: Number(day.totalRooms || 0),
          })
        }
      }
                  ...
                  {deluxeTotal > 0 && (
                    <td className="px-4 py-2.5 text-center">
                      <span className={`font-bold ${d.deluxeAvail === 0 ? 'text-red-400' : d.deluxeAvail === 1 ? 'text-orange-400' : 'text-blue-400'}`}>
                        {d.deluxeAvail === 0 ? '❌ Full' : `${d.deluxeAvail} avail`}
                      </span>
                    </td>
                  )}
                  {premiumTotal > 0 && (
                    <td className="px-4 py-2.5 text-center">
                      <span className={`font-bold ${d.premiumAvail === 0 ? 'text-red-400' : d.premiumAvail === 1 ? 'text-orange-400' : 'text-[#c9a14a]'}`}>
                        {d.premiumAvail === 0 ? '❌ Full' : `${d.premiumAvail} avail`}
                      </span>
                    </td>
                  )}
                </tr>
...
function BookingContent() {
  ...
  // Step 1
  const [checkIn,  setCheckIn]    = useState(sp.get('checkIn')  || checkInDate || '')
  const [checkOut, setCheckOut]   = useState(sp.get('checkOut') || checkOutDate || '')
  const [numRooms, setNumRooms]   = useState(1)
  const [adults,   setAdults]     = useState(2)
  // Meal plan driven by URL param
  const [mealPlan, setMealPlan] = useState<PublicMealPlan>(() => {
    const mp = sp.get('meal')
    return (mp === 'EP' || mp === 'CP') ? mp : 'CP'
  })
  ...
  useEffect(() => {
    if (!sp.get('checkIn') && checkInDate && !checkIn) setCheckIn(checkInDate)
    if (!sp.get('checkOut') && checkOutDate && !checkOut) setCheckOut(checkOutDate)
  }, [checkInDate, checkOutDate, checkIn, checkOut, sp])
  ...
  useEffect(() => {
    ...
    fetch('/api/get-room-pricing', {
      ...
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(result.error || 'Price not available')
        setPricing(result)
      })
      .catch(() => {
        setPricing(null)
  ...
  useEffect(() => {
    fetch('/api/auth/me', { cache: 'no-store' })
      ...
      .then((result) => {
        ...
        if (!authUser || result?.requires_profile_completion) return
        setHasUserProfile(true)
        setUserId(authUser.id)
        if (authUser.email) setEmail(authUser.email)
        if (authUser.name) setName(authUser.name)
        if (authUser.phone) {
          const stored = String(authUser.phone)
  ...
  const cgstDisplay   = Math.round(subtotalExGst * 0.025 * 100) / 100
  const sgstDisplay   = Math.round(subtotalExGst * 0.025 * 100) / 100
  const gst     = cgstDisplay + sgstDisplay
  const total   = Math.round((subtotalExGst + gst) * 100) / 100
  ...
  function validateStep2() {
    const ne = validateName(name), pe = validatePhoneIntl(phone, countryCode), ee = validateEmail(email)
  ...
  async function checkAvailability() {
    ...
    if (nights > MAX_ONLINE_STAY_NIGHTS) { setErrors(p => ({ ...p, checkOut: `Online booking is available for up to ${MAX_ONLINE_STAY_NIGHTS} nights only` })); return }
  ...
  async function handlePayment() {
    ...
    try {
      ...
      const rzp = new (window as any).Razorpay({
        ...
        description: `${room!.name} — ${nights}N · ${MEAL_INFO[mealPlan].label}`,
        ...
        handler: async (resp: any) => {
          ...
          const cacheConfirmedBooking = (booking: any) => {
        ...
        modal: {
          ondismiss: async () => {
            ...
            setPaying(false)
            toast('If amount was deducted, please do not pay again immediately. You can re-check payment status shortly.', { icon: 'ℹ️' })
          },
        },
      })
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="flex gap-4 p-5 rounded-2xl border border-white/8" style={{ background: 'rgba(255,255,255,0.03)' }}>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {PUBLIC_MEAL_PLANS.map(mp => {
                    const stayTotal = getMealStayTotal(mp)
                    const active = mealPlan === mp
                    const info = PUBLIC_MEAL_INFO[mp]
                    return (
                      <button key={mp} onClick={() => setMealPlan(mp)}
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ` +
                          (active ? 'border-[#c9a14a] bg-[#c9a14a]/15 text-[#c9a14a]' : 'border-white/10 text-white/40 hover:border-white/25 hover:text-white/70')}>
                        {info.icon} {info.label}
                        <span className="font-bold ml-1">
                          {checkIn && checkOut && stayTotal > 0 ? `₹${stayTotal.toLocaleString()}` : 'Select dates'}
                        </span>
                      </button>
                    )
                  })}
              ...
              <>
                <div className="p-6 rounded-2xl border border-white/8 space-y-6" style={{ background: 'rgba(255,255,255,0.03)' }}>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    >
                      <label
                        className="block cursor-pointer text-sm font-medium mb-2"
                    ...
                    >
                      ...
                      <input ref={checkOutInputRef} type="date" value={checkOut} min={checkIn ? addDays(checkIn, 1) : tomorrow}
                  ...
                  {nights > MAX_ONLINE_STAY_NIGHTS ? (
                    <div className="rounded-xl border border-[#c9a14a]/20 px-4 py-3 text-xs text-[#c9a14a]/80" style={{ background: 'rgba(201,161,74,0.06)' }}>
                      For stays longer than 4 days, please contact the Leafwalk team directly.
                    </div>
                  ) : null}
                  ...
                  <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[#c9a14a]/20" style={{ background: 'rgba(201,161,74,0.05)' }}>
                    <span className="text-2xl">{PUBLIC_MEAL_INFO[mealPlan].icon}</span>
                    <div>
                      <div className="text-white text-sm font-semibold">{PUBLIC_MEAL_INFO[mealPlan].label}</div>
                      <div className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.45)' }}>
                        {PUBLIC_MEAL_INFO[mealPlan].desc}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="lg:col-span-1">
            <BookingSummary
              room={{ name: room.name, featured_image: room.featured_image }}
              checkIn={checkIn}
              checkOut={checkOut}
              nights={nights}
              numRooms={numRooms}
              adults={adults}
              mealPlan={mealPlan}
              mealLabel={PUBLIC_MEAL_INFO[mealPlan].label}
              autoXbeds={autoXbeds}
              nightlyBreakdown={stayTariff.nights}
