...
import { buildBookingNumber } from '@/lib/reference-numbers'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!room_id || !check_in || !check_out || !nights || !guest_name || !guest_phone || !guest_email) {
    ...
    const expectedNights = Math.ceil((new Date(`${check_out}T12:00:00Z`).getTime() - new Date(`${check_in}T12:00:00Z`).getTime()) / 86400000)
    ...
    if (availability.availableRooms < numRooms) {
      return NextResponse.json({
        error: `Only ${availability.availableRooms} ${availability.room.category} room(s) available for selected dates`
      }, { status: 409 })
    }
    ...
    if (ratesError || mealError) {
      return NextResponse.json({ error: 'Could not fetch room rates' }, { status: 500 })
    }

    let pricingMatrix
    let mealUnitTotals
    try {
      pricingMatrix = buildLwwebPricingMatrix({
      ...
      mealUnitTotals = buildMealUnitTotals({
        checkIn: String(check_in),
        checkOut: String(check_out),
        mealPrices: ((mealRows || []) as any[]).map((meal) => ({
          meal_type: meal.meal_type,
      ...
      return NextResponse.json({ error: pricingError.message || 'No website tariff found for the selected dates' }, { status: 400 })
    }
    ...
    const mealAddonPerAdult = selectedMealPlan === 'CP' ? round2(Number(mealUnitTotals?.breakfast || 0)) : 0
    ...
    const { data: booking, error: bErr } = await supabase
      .from('bookings').insert({
        ...
        advance_amount: 0,
        balance_amount: 0,
        user_id:         auth?.user?.email && String(auth.user.email).toLowerCase() === normalizedEmail ? auth.userId : null,
    ...
    if (bErr || !booking) {
      logError('Booking insert error:', bErr)
    ...
    if (!booking.booking_number) {
      ...
      const { data: numberedBooking, error: numberErr } = await supabase
        .from('bookings')
      ...
      if (!numberErr && numberedBooking) {
        finalBooking = numberedBooking
      }
    }

    if (isLocalTestMode()) {
      logDebug('LOCAL TEST MODE public website booking created; evaluating email triggers', {
        booking_id: finalBooking.id,
        booking_source: finalBooking.booking_source || 'website',
        booking_status: finalBooking.booking_status || 'pending',
      })
    }
    await sendBookingLifecycleEmails(finalBooking.id, 'public_booking_created')
    ...
    logError('Booking create error:', err)
    if (err.message === 'Room not found' || err.message === 'Room not available') {
      return NextResponse.json({ error: err.message }, { status: 404 })
    }
    if (err.message === 'Could not fetch room inventory' || err.message === 'Could not check availability') {
