...
const CONTACT_CARDS = [
  { icon: 'Location', title: 'Our Location', lines: ['Vill- Banas, Narad Chatti', 'Hanuman Chatti, Yamunotri Road', 'Uttarkashi, Uttarakhand - 249193'] },
  { icon: 'Call', title: 'Call Us', lines: ['+91 9368080535', '+91 8630227541'], links: [{ href: 'tel:+919368080535', label: 'Call Now' }] },
  { icon: 'WhatsApp', title: 'WhatsApp', lines: ['+91 8630227541', 'Available 8 AM - 10 PM'], links: [{ href: 'https://wa.me/918630227541?text=Hello%20LeafWalk%20Resort', label: 'Open WhatsApp' }] },
  { icon: 'Email', title: 'Email', lines: ['info@leafwalk.in'], links: [{ href: 'mailto:info@leafwalk.in', label: 'Send Email' }] },
]
...
export default function ContactPage() {
  ...
  function validate(): Record<string, string> {
    ...
    if (!PHONE_RE.test(form.phone.replace(/\D/g, ''))) nextErrors.phone = 'Enter a valid 10-digit Indian mobile number'
    if (form.email && !EMAIL_RE.test(form.email)) nextErrors.email = 'Enter a valid email address'
    if (!form.checkin) nextErrors.checkin = 'Check-in date required'
    if (!form.checkout) nextErrors.checkout = 'Check-out date required'
    if (form.checkin && form.checkout && new Date(form.checkout) <= new Date(form.checkin)) nextErrors.checkout = 'Check-out must be after check-in'
    return nextErrors
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const allTouched = Object.fromEntries(Object.keys(form).map((key) => [key, true]))
    ...
    try {
      const payload = {
        name: form.name.trim(),
        email: form.email.trim() || 'no-email@leafwalk.local',
        phone: `+91 ${form.phone}`,
        subject: `Website enquiry for ${form.rooms} room(s)`,
        message: [
          `Preferred stay: ${form.checkin} to ${form.checkout}`,
          `Rooms requested: ${form.rooms}`,
          form.message.trim() ? `Guest note: ${form.message.trim()}` : null,
        ].filter(Boolean).join('\n'),
      }
  ...
  useEffect(() => {
    if (success) {
      const timeout = setTimeout(() => setSuccess(false), 6000)
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-16">
          {CONTACT_CARDS.map((card) => (
            <div key={card.title} className="p-5 rounded-2xl border border-white/8 hover:border-[#c9a14a]/25 transition-all" style={{ background: 'rgba(255,255,255,0.03)' }}>
              <span className="text-xl block mb-3 text-[#c9a14a]">{card.icon}</span>
              <h3 className="text-white font-semibold mb-2 text-sm">{card.title}</h3>
              {card.lines.map((line) => <p key={line} className="text-white/45 text-xs leading-relaxed">{line}</p>)}
              {card.links?.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" autoComplete="name" maxLength={100} placeholder="Your full name" value={form.name} onChange={(e) => setForm((state) => ({ ...state, name: e.target.value }))} onBlur={() => touch('name')} className={INP('name')} style={BASE} />
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="tel" autoComplete="tel" maxLength={10} placeholder="10-digit number" inputMode="numeric" value={form.phone} onChange={(e) => setForm((state) => ({ ...state, phone: e.target.value.replace(/\D/g, '').slice(0, 10) }))} onBlur={() => touch('phone')} className={`${INP('phone')} pl-12`} style={BASE} />
                  </div>
                ...
                <div>
                  ...
                  <input type="email" autoComplete="email" maxLength={100} placeholder="your@email.com" value={form.email} onChange={(e) => setForm((state) => ({ ...state, email: e.target.value }))} onBlur={() => touch('email')} className={INP('email')} style={BASE} />
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input type="date" min={today} value={form.checkin} onChange={(e) => setForm((state) => ({ ...state, checkin: e.target.value }))} onBlur={() => touch('checkin')} className={INP('checkin')} style={{ ...BASE, colorScheme: 'dark' }} />
                ...
                <div>
                  ...
                  <input type="date" min={form.checkin || tomorrow} value={form.checkout} onChange={(e) => setForm((state) => ({ ...state, checkout: e.target.value }))} onBlur={() => touch('checkout')} className={INP('checkout')} style={{ ...BASE, colorScheme: 'dark' }} />
                ...
                <div>
                  ...
                  <select value={form.rooms} onChange={(e) => setForm((state) => ({ ...state, rooms: e.target.value }))} className="w-full px-4 py-3 rounded-xl border border-white/15 text-sm focus:outline-none focus:border-[#c9a14a] transition-all" style={BASE}>
                    {['1', '2', '3', '4', '5'].map((num) => <option key={num} value={num} style={{ background: '#1a1a1a' }}>{num} Room{Number(num) > 1 ? 's' : ''}</option>)}
                  </select>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea rows={4} maxLength={1000} placeholder="Tell us about your trip - group size, occasion, preferences..." value={form.message} onChange={(e) => setForm((state) => ({ ...state, message: e.target.value }))} className="w-full px-4 py-3 rounded-xl border border-white/15 text-sm resize-none focus:outline-none focus:border-[#c9a14a] transition-all hover:border-white/25" style={BASE} />
                <p className="text-xs text-right mt-1" style={{ color: 'rgba(255,255,255,0.25)' }}>{form.message.length}/1000</p>
              </div>

              <button type="submit" disabled={submitting} className="w-full py-4 rounded-xl font-bold text-sm transition-all hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed" style={{ background: 'linear-gradient(135deg, #c9a14a, #e6c87a)', color: '#000' }}>
                {submitting ? (
              ...
              {success && (
                <div className="p-4 rounded-xl text-sm font-medium text-center" style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.25)', color: '#4ade80' }}>
                  Thank you! Your enquiry has been sent. Our team will contact you soon.
                </div>
              )}
            </form>
          </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="space-y-4">
                {[
                  { title: 'By Road', desc: '180 km from Rishikesh via Uttarkashi on NH-94. Taxi and buses available from Rishikesh, Dehradun, and Uttarkashi.' },
                  { title: 'By Train', desc: 'Nearest railway station: Rishikesh (180 km) or Haridwar (190 km). Hire a cab from the station.' },
                  { title: 'By Air', desc: 'Nearest airport: Jolly Grant Airport, Dehradun (200 km). Cabs available at the airport.' },
                  { title: 'Resort Pickup', desc: 'We arrange pickup from Uttarkashi town on request. Contact us in advance to book.' },
                ].map((item) => (
