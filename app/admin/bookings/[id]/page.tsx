...
export default function BookingDetailPage() {
  ...
  async function loadBooking(token?: string) {
    try {
      ...
      setBooking(data)
      await loadPaymentLedger(tok || await getToken(), data.id)
    } catch (e) {
  ...
  async function loadPaymentLedger(token: string, bId?: string) {
    const id = bId || bookingId
    setLedgerLoading(true)
    try {
      const res = await fetch(`/api/admin/data?type=payment-ledger&booking_id=${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      const d = await res.json()
      setPaymentLedger(d.data || [])
    } catch {}
  ...
  async function confirmHold() {
    ...
    try {
      ...
      const body: any = {
        ...
        payment_status: advance >= total ? 'fully_paid' : advance > 0 ? 'payment_processing' : 'pending',
        ...
        payment_due_date: advance > 0 && advance < total ? dueDate || null : null,
      }
      ...
      if (advance > 0) {
        await fetch('/api/admin/data?type=payment-ledger', {
          ...
          body: JSON.stringify({
            ...
            payment_type: advance >= total ? 'final' : 'advance',
            amount: advance,
            payment_method: confirmPayment.method,
            payment_ref: confirmPayment.transaction_number || null,
            payment_date: confirmPayment.payment_date,
            payment_due_date: advance < total ? dueDate || null : null,
            notes: confirmPayment.payment_notes || null,
          })
        })
      }
  ...
  async function recordBalancePayment() {
    ...
    try {
      ...
      const body: any = {
        ...
        payment_status: isFullyPaid ? 'fully_paid' : 'payment_processing',
      ...
      await fetch('/api/admin/data?type=payment-ledger', {
        ...
        body: JSON.stringify({
          ...
          amount: paid,
          payment_method: balancePayment.method,
          payment_ref: balancePayment.transaction_number || null,
          payment_date: balancePayment.payment_date,
          notes: balancePayment.payment_notes || null,
        })
      })
  ...
  async function sendTestBalanceReminder() {
    ...
    try {
      ...
      toast.success(result.email ? 'Balance reminder test mail sent' : 'Reminder request completed, but email provider did not confirm send')
    } catch (error: any) {
      toast.error(error.message || 'Failed to send balance reminder')
    } finally {
            ...
            <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-4">
                  ...
                  {confirmPayment.advance > 0 && confirmPayment.advance < Number(booking.total_amount) && (
                    <div>
                      <label className="text-white/60 text-sm mb-2 block">Balance Due Date</label>
                      <input
                        type="date"
                        value={dueDate}
                        min={confirmPayment.payment_date || new Date().toISOString().split('T')[0]}
                        onChange={e => setDueDate(e.target.value)}
                        className="w-full px-4 py-3 bg-[#111] border border-white/20 rounded-lg text-white"
                      />
                      <p className="text-white/30 text-xs mt-2">Optional follow-up date for the remaining balance.</p>
                    </div>
                  )}
          ...
          {Number(booking.balance_amount) > 0 && !['cancelled','no_show'].includes(booking.booking_status) && (
            <div className="mt-6 bg-orange-500/5 border border-orange-500/20 rounded-2xl p-6">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={sendTestBalanceReminder}
                  disabled={sendingBalanceReminder}
                  className="mr-3 px-5 py-2.5 bg-yellow-500/15 hover:bg-yellow-500/25 text-yellow-300 border border-yellow-500/25 rounded-full text-sm font-semibold transition-all whitespace-nowrap disabled:opacity-50">
                  {sendingBalanceReminder ? 'Sending...' : 'Test Balance Mail'}
                </button>
                <button
                  onClick={() => {
          ...
          <div className="mt-6 bg-white/5 border border-white/10 rounded-2xl p-6">
            ...
            {booking.booking_source === 'tour_operator' && (
              <p className="mt-3 text-white/35 text-xs">
                Check In Pass guest copy hai; isme tariff, GST, advance aur balance details print nahi hoti.
              </p>
            )}
          </div>
